...
def train_anomaly_model(db: Session = Depends(get_db)):
    """Administrative endpoint to train the IsolationForest model and evaluate"""
    ...
    # 2. Train and flag anomalies
    df_result = detector.train(df)
    
    # 3. Save model
    detector.save_model()
    
    # 4. Evaluation printouts
    ...
    # Top 10 anomalies (most negative scores)
    ...
    return {
        ...
        "dataset_size": initial_len,
        "anomalies_detected": num_anomalies,
        "contamination_rate_effective": f"{contamination_rate:.2%}",
